...
def filter_messages_for_prompt(messages: List[Dict[str, str]]) -> List[Dict[str, str]]:
    """Remove assistant messages; keep system/user/"""
...
def token_ids_for_strings(tokenizer, strings: List[str]) -> List[int]:
    ...
    for s in strings:
        ...
        if enc:
            ids.append(enc[-1])  # take last token id
...
def parse_letters_from_text(text: str, valid_letters: List[str], multi: bool) -> List[str]:
    """
    STRICT parser for MCQ predictions.
    """
    if text is None:
        return []
    s = text.upper().strip()
...
def label_token(tokenizer, tid: int, eos_id: Optional[int]) -> str:
    ...
    if eos_id is not None and tid == eos_id:
        return "<eos>"
    # Try to decode a single id (don’t skip specials so we see them)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate on MCQ/OEQA datasets.")
