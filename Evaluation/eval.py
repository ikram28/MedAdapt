"""
Evaluate a causal LM on MCQ and OEQA datasets.
...
def example_to_prompt_text(ex: Dict[str, Any]) -> str:
    """STRICT: only accept user_prompt; error otherwise."""
    up = ex.get("user_prompt")
    if isinstance(up, str) and up.strip():
        return up
    raise ValueError("Example missing a non-empty 'user_prompt' string.")

def tokenize_plain(tokenizer, text: str) -> torch.LongTensor:
...
def token_ids_for_strings(tokenizer, strings: List[str]) -> List[int]:
    ...
    for s in strings:
        ...
        if enc:
            ids.append(enc[-1])  # last token id
...
def normalize_mcq_answer(ans: str | List[str]) -> List[str]:
    items = ans if isinstance(ans, list) else re.split(r"[\s,]+", ans)
...
def hamming_score(true_labels: List[str], pred_labels: List[str]) -> float:
    set_t = set(true_labels); set_p = set(pred_labels)
...
def step_probs_from_scores_list(scores_list, tokid_to_letter, letter_token_ids):
...
def evaluate_file(model, tokenizer, path: str, out_dir: str, device: str) -> Tuple[str, Dict[str, float]]:
    ...
    for ex in items:
        ...
        # STRICT: only user_prompt
        prompt_text = example_to_prompt_text(ex)
        prompt_ids = tokenize_plain(tokenizer, prompt_text).to(device)

        # Max new tokens heuristic
        if kind == "qcmu":
            max_new = 1
        elif kind == "qcm":
        ...
        if classes and kind in {"qcmu", "qcm"}:
            ...
            # Tokens that can realize a letter at step 0:
            # - bare "A"/"B"/...
            # - or single token "▁A"/"▁B"/... (encoded from " A"/" B"/...)
            letter_ids = token_ids_for_strings(tokenizer, letters + [" " + l for l in letters])

            # Separators are only useful for multi-answer QCM; keep your old list
            sep_ids = token_ids_for_strings(tokenizer, [",", ", ", " ", ",\n"])

            # Stopping ids (EOS/EOT etc.)
            stop_ids: List[int] = []
            ...
            # ---------- CHANGED: allowed set depends on kind ----------
            if kind == "qcmu":
                # Single-answer MCQ: allow ONLY letters (+ stop) so the first token MUST be a class.
                allowed_for_gen = sorted(set(letter_ids + stop_ids))
            else:
                # Multi-answer MCQ: keep your original behavior (letters + separators + stop).
                allowed_for_gen = sorted(set(letter_ids + sep_ids + stop_ids))
            # ----------------------------------------------------------

            # Build id -> label for all allowed tokens (letters, seps, EOS/EOT)
            ...
            # Probabilities & per-step traces (unchanged)
            ...
            # Per-step probabilities over ALL allowed tokens
            ...
            if kind == "qcmu":
                ...
                if not pred_constrained and class_probabilities:
                    # fallback: simulate “letters-only at step 0”
                    pred_constrained = [max(class_probabilities, key=class_probabilities.get)]
...
def step_probs_over_allowed(scores_list, allowed_token_ids, id2label) -> List[Dict[str, float]]:
...
def append_summary_csv(csv_path: str, dataset_name: str, kind: str, summary: Dict[str, float]):
    header = [
        "dataset", "type", "n_items",
        "em_greedy", "hamming_greedy", "em_constrained", "hamming_constrained",
    ]
    row = [
        dataset_name,
        kind,
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Causal LM")
